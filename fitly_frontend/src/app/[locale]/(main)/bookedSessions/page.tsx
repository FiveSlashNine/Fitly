...
function BookedSessionsPage() {
  ...
  const t = useTranslations("BookedSessionsPage");
            ...
            {sessions.map((session) => (
              <li key={session.id}>
                <SessionCard
                  session={session}
                  buttonLabel={
                    userId !== -1 ? t("button_cancel_booking") : undefined
                  }
                  action={
                    userId !== -1
                      ? async () => {
...
export default function BookedSessionsWrapper() {
  const t = useTranslations("BookedSessionsPage");
  return (
    <Suspense fallback={<div>{t("pagination_loading")}...</div>}>
      <BookedSessionsPage />
    </Suspense>
  );
}

...
public class SessionService {
    ...
    public Page<Session> getSessions(String location, SessionType type, Status status, String searchQuery, Long userId, Boolean enrolledOnly, Boolean ownedGymOnly, Pageable pageable) {
        ...
        if (userId != null) {
            if (Boolean.TRUE.equals(ownedGymOnly)) {
                spec = spec.and(SessionSpecification.hasGymOwner(userId));
            } else if (enrolledOnly) {
                spec = spec.and(SessionSpecification.hasSessionHolder(userId));
            } else {
                spec = spec.and(SessionSpecification.hasNoSessionHolder(userId));
            }
        }
